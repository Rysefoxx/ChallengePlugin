...
public class ConnectionManager implements IDatabaseService {
    ...
    public void onEnable(@NotNull ChallengePlugin plugin) {
        ...
        ServiceRegistry.registerService(ConnectionManager.class, this);

        DatabaseTableManager tableManager = new DatabaseTableManager(this);
        tableManager.createDefaultTables();
    }
    ...
    private void setupHikariCP() {
        ...
        if (!isValidConfig(config)) {
            ChallengePlugin.logger().severe("Failed to load database.yml!");
    ...
    public @Nullable PreparedStatement prepareStatement(@Nullable Connection connection, @NotNull String query) throws SQLException {
        if (connection == null) {
            ChallengePlugin.logger().severe("The connection for the prepared statement is null!");
            return null;
        }
